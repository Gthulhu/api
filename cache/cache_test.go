...
func TestStrategyCache_ShouldReturnCachedWhenNoChanges(t *testing.T) {
	// Arrange
	cache := cache.NewStrategyCache()
	initialPods := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
		{PodUID: "pod2", Processes: []domain.PodProcess{{PID: 200, Command: "test"}}},
	}
	inputStrategies := []*domain.SchedulingStrategy{
		{Priority: true, ExecutionTime: 1000, Selectors: []domain.LabelSelector{{Key: "app", Value: "test"}}},
	}
	initialStrategies := []*domain.SchedulingStrategy{
...
func TestStrategyCache_ShouldInvalidateOnNewPod(t *testing.T) {
	...
	updatedPods := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
		{PodUID: "pod2", Processes: []domain.PodProcess{{PID: 200, Command: "test"}}}, // New pod
	}
	inputStrategies := []*domain.SchedulingStrategy{
		{Priority: true, ExecutionTime: 1000, Selectors: []domain.LabelSelector{{Key: "app", Value: "test"}}},
	}
	...
	cache.UpdatePodSnapshot(initialPods)
	cache.UpdateStrategySnapshot(inputStrategies)
	cache.SetStrategies([]*domain.SchedulingStrategy{{Priority: true, ExecutionTime: 1000, PID: 100}})
...
func TestStrategyCache_ShouldInvalidateOnPodRestart(t *testing.T) {
	...
	restartedPods := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 150, Command: "test"}}},
	}
	inputStrategies := []*domain.SchedulingStrategy{
		{Priority: true, ExecutionTime: 1000, Selectors: []domain.LabelSelector{{Key: "app", Value: "test"}}},
	}
...
func TestStrategyCache_ShouldNotInvalidateOnIrrelevantChanges(t *testing.T) {
	...
	initialPods := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
		{PodUID: "pod2", Processes: []domain.PodProcess{{PID: 200, Command: "other"}}},
	}
	// Same pods, just different order
	reorderedPods := []*domain.PodInfo{
		{PodUID: "pod2", Processes: []domain.PodProcess{{PID: 200, Command: "other"}}},
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
	}
	inputStrategies := []*domain.SchedulingStrategy{
...
func TestStrategyCache_ShouldExpireAfterTTL(t *testing.T) {
	// Arrange
	cache := cache.NewStrategyCacheWithTTL(100 * time.Millisecond) // Short TTL for testing
	pods := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
	}
	inputStrategies := []*domain.SchedulingStrategy{
...
func TestStrategyCache_ShouldInvalidateOnStrategyChange(t *testing.T) {
	...
	updatedStrategies := []*domain.SchedulingStrategy{
		{Priority: false, ExecutionTime: 2000, Selectors: []domain.LabelSelector{{Key: "app", Value: "prod"}}}, // Changed strategy
	}
...
func TestPodChangeDetector_ComputeFingerprint(t *testing.T) {
	// Arrange
	detector := cache.NewPodChangeDetector()
	pods1 := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
	}
	pods2 := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 100, Command: "test"}}},
	}
	pods3 := []*domain.PodInfo{
		{PodUID: "pod1", Processes: []domain.PodProcess{{PID: 200, Command: "test"}}}, // Different PID
	}
...
func TestKubernetesPodWatcher_ShouldDetectPodEvents(t *testing.T) {
	...
	watcher.SimulateEvent(cache.PodEvent{
		Type: "ADDED",
		Pod: apiv1.Pod{
