...
-----END RSA PRIVATE KEY-----`
	privateKey, err := InitRSAPrivateKey(rsaPem)
	require.NoError(t, err, "Failed to initialize RSA private key")
	require.NotNil(t, privateKey)
	t.Log(privateKey.PublicKey)

	publicPem, err := RSAPublicKeyToPEM(&privateKey.PublicKey)
	require.NoError(t, err)
	require.NotNil(t, publicPem)
	t.Log(string(publicPem))

	publicKey, err := PEMToRSAPublicKey(string(publicPem))
	require.NoError(t, err)
